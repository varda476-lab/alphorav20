...
export default function CinematicPreloader({ onComplete }: CinematicPreloaderProps) {
  ...
  return (
    ...
    >
      ...
      {isGlitching && (
        <div className="absolute inset-0 bg-brand-cyan/5 z-50 pointer-events-none animate-glitch border-4 border-brand-cyan/20" />
      )}

      {/* Main Singularity Element */}
      <div className="relative w-80 h-80 flex items-center justify-center">
        
        {/* Neon Red Ring 1 (Clockwise) */}
        <motion.div
          animate={isInitialized ? { scale: 3.5, opacity: 0 } : { scale: 1 }}
          transition={{ duration: 2.2, ease: 'easeInOut' }}
          className="absolute inset-0 border border-brand-cyan/40 rounded-full animate-rotate-clockwise shadow-[0_0_20px_rgba(255,62,62,0.2)]"
          style={{ transformStyle: 'preserve-3d', rotateX: '65deg', rotateY: '15deg' }}
        />

        {/* Neon Yellow Ring 2 (Counter-Clockwise) */}
        <motion.div
          animate={isInitialized ? { scale: 3.8, opacity: 0 } : { scale: 1 }}
          transition={{ duration: 2.2, ease: 'easeInOut' }}
          className="absolute inset-2 border border-brand-purple/30 rounded-full animate-rotate-counter shadow-[0_0_30px_rgba(250,204,21,0.15)]"
          style={{ transformStyle: 'preserve-3d', rotateX: '65deg', rotateY: '-15deg' }}
        />

        {/* Deep Red Glowing Singularity Orb */}
        <motion.div
